...
class Options {
  constructor (options) {
    ...
    if (typeof this.options.attributes !== 'undefined' && Array.isArray(this.options.attributes)) {
    ...
    this.cmd = yargs
      .option('backend', {
        alias: 'b',
        default: this.options.backend || 'html5',
        describe: 'set output format backend',
        type: 'string'
      })
      .option('doctype', {
        alias: 'd',
        default: this.options.doctype || 'article',
      ...
      .option('safe-mode', {
        alias: 'S',
        default: this.options.safe || 'unsafe',
...
By default, the output is written to a file with the basename of the source file and the appropriate extension`)
  ...
  parse (argv) {
    ...
    this.argv = argv
    this.args = this.argsParser().parse(processArgs)
    ...
    const options = Options.convertOptions(this.args, this.attributes)
  ...
  static convertOptions (args, attrs) {
