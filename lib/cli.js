...
function outputResult (data, file, cliOptions) {
  var backend = cliOptions['backend'];
  ...
  if (outFile === '') {
    ...
    if (outFile == null) {
      ...
      outputFile = outDir + path.sep + path.basename(file, extname) + outputExtname;
    } else {
      outputFile = outDir + path.sep + outFile;
    }
  ...
  cli.main(function (cliArgs, cliOptions) {
    ...
    if (displayVersion) {
      ...
      cliArgs.forEach(function (file) {
        var data = convertFile(file, options);
        outputResult(data, file, cliOptions);
      });
    }
  });
}
